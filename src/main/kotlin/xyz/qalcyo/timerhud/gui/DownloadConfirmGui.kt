...
class DownloadConfirmGui(private val parent: GuiScreen?) : GuiScreen() {
        ...
        when (button.id) {
            0 -> {
                ...
                Multithreading.runAsync {
                    if (Updater.download(
                            updateUrl,
                            File("mods/${TimerHUD.NAME}-${Updater.latestTag.substringAfter("v")}.jar")
                        ) && Updater.download(
                        ...
                        EssentialAPI.getNotifications()
                            .push(TimerHUD.NAME, "The ingame updater has successfully installed the newest version.")
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        drawCenteredString(
            fontRendererObj,
            EnumChatFormatting.DARK_PURPLE.toString() + TimerHUD.NAME,
            width / 4,
            3,
            -1
        )
